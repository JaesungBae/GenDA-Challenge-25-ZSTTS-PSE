...
import torch
from asteroid.losses.sdr import singlesrc_neg_sisdr, singlesrc_neg_snr
from numpy.random import Generator
from utils.exp_utils import ExperimentError
...
def _make_2d(x: torch.Tensor):
    ...
    if x.ndim == 1:
        ...
        if x.ndim != 2:
            raise ValueError("Could not force 2d.")
        return x
...
def _make_3d(x: torch.Tensor):
    ...
    if x.ndim == 1:
        ...
        if x.ndim != 3:
            raise ValueError("Could not force 3d.")
        return x


def mix_signals(source: np.ndarray, noise: np.ndarray, snr_db: Union[float, np.ndarray]) -> np.ndarray:
    ...
    b = np.sqrt((energy_s / energy_n) * (10 ** (-snr_db / 10.0)))
    return source + b * noise


def sparsity_index(signal: np.ndarray) -> float:
...
def wav_read(filepath: Union[str, os.PathLike]) -> np.ndarray:
...
def wav_write(filepath: Union[str, os.PathLike], array: np.ndarray):
...
def wav_sample(data: np.ndarray, num_clips: int, seed: Optional[int] = None) -> np.ndarray:
    rng = np.random.default_rng(seed)
...
def sisdr(estimate: torch.Tensor, target: torch.Tensor, reduction: Optional[str] = None) -> torch.Tensor:
...
def sisdr_improvement(
    ...
    return sdr_improvement(estimate, target, mixture, reduction=reduction, scale_invariant=True)
...
def sdr_improvement(
    ...
    output = sdr(estimate, target, scale_invariant=scale_invariant) - sdr(
        mixture, target, scale_invariant=scale_invariant
    )
    if reduction == "mean":
        output = torch.mean(output)
    return output
...
def dataframe_librispeech(
    ...
    valid_subsets = ["train-clean-100", "train-clean-360", "dev-clean", "test-clean"]
    if not dataset_directory.joinpath("dataframe.csv").exists():
        ...
        if not len(rows):
            raise (f"Could not find any .WAV files within " f"{dataset_directory}.")
        df = pd.DataFrame(rows, colValueErrorumns=columns)
        df.to_csv(
            dataset_directory.joinpath("dataframe.csv"),
            header=columns,
            index=False,
            index_label=False,
        )
    ...
    df["split"] = pd.Categorical(df["split"], ["pretrain", "preval", "train", "val", "test"])
    ...
    df = df[["filepath", "subset_id", "speaker_id", "split", "duration", "sparsity"]]
...
def dataframe_demand(
    ...
    if not dataset_directory.joinpath("dataframe.csv").exists():
        ...
        columns = ["category_id", "location_id", "filepath", "duration", "sparsity"]
        for filepath in sorted(dataset_directory.rglob("*.wav")):
            if "ch01" not in filepath.stem:
                continue
            category_id = [_ for _ in valid_categories if _[0].upper() == filepath.parent.stem[0].upper()][0]
            location_id = [_ for _ in valid_locations if filepath.parent.stem[1:].upper() in _.upper()][0]
            y = wav_read(filepath)
            duration = np.size(y) / sample_rate
            sparsity = sparsity_index(y)
            rows.append((category_id, location_id, str(filepath), duration, sparsity))
        if not len(rows):
            raise ValueError(f"Could not find any .WAV files within " f"{dataset_directory}.")
...
def dataframe_fsd50k(
    ...
    if not dataset_directory.joinpath("dataframe.csv").exists():
        ...
        for row in df.itertuples():
            subdir = "FSD50K.eval_audio" if row.split == "test" else "FSD50K.dev_audio"
...
def dataframe_musan(
    ...
    if not dataset_directory.joinpath("dataframe.csv").exists():
        rows = []
        columns = ["split", "filepath", "duration", "sparsity"]
        for filepath in sorted(dataset_directory.rglob("*.wav")):
...
def dataframe_irsurvey(
    ...
    for filepath in files:
        ...
        if filepath.name == "h053_Office_ConferenceRoom_stxts.wav":
            ...
            try:
                ir_frequency = int(filepath.name.split("txt")[0].split("_")[-1])
            except (Exception,):
                ir_frequency = 1
        rows += [(ir_name, ir_frequency, ir_duration, str(filepath))]

    df = pd.DataFrame(rows, columns=["name", "frequency", "duration", "filepath"])
    ...
    # organize by split
    df["split"] = "train"
    df.loc[int(270 * 0.8) : int(270 * 0.9), "split"] = "val"
    df.loc[int(270 * 0.9) :, "split"] = "test"
...
def dataframe_slr28(
    ...
    def parse_rir_list(filepath: str, real: bool = False):
        ...
        for line in open(fp).read().splitlines():
            ...
            i_filepath = pathlib.Path(dataset_directory).joinpath(str(parts[4]).replace("RIRS_NOISES/", ""))
    ...
    df_real = pd.DataFrame(rows, columns=["type", "id", "room", "filepath"])

    # shuffle the recordings
    df_real = df_real.sample(frac=1, random_state=0).reset_index(drop=True)
    nrows = len(df_real)

    # organize by split
    df_real["split"] = "train"
    df_real.loc[int(nrows * 0.8) : int(nrows * 0.9), "split"] = "val"
    df_real.loc[int(nrows * 0.9) :, "split"] = "test"
    ...
    df_synth = pd.DataFrame(rows, columns=["type", "id", "room", "filepath"])

    # shuffle the recordings
    df_synth = df_synth.sample(frac=1, random_state=0).reset_index(drop=True)
    nrows = len(df_synth)

    # organize by split
    df_synth["split"] = "train"
    df_synth.loc[int(nrows * 0.8) : int(nrows * 0.9), "split"] = "val"
    df_synth.loc[int(nrows * 0.9) :, "split"] = "test"
...
def split_speakers(only_tc100: bool = True) -> Tuple[List, List, List]:
...
class Mixtures:
    ...
    def __init__(
        ...
        frac_speech: Optional[float] = 1.0,
        ...
        if isinstance(snr_premixture, tuple):
            ...
            raise ValueError("Expected `snr_premixture` to be a float type or " "a tuple of floats.")
        if isinstance(snr_mixture, tuple):
            ...
            raise ValueError("Expected `snr_mixture` to be a float type or " "a tuple of floats.")
        ...
        if not (split_speech in ("all", "pretrain", "preval", "train", "val", "test")):
            raise ValueError(
                'Expected `split_speech` to be either "all", ' '"pretrain", "preval", "train", "val", or "test".'
            )
        if snr_premixture is not None:
            if not (split_premixture in ("train", "val", "test")):
                raise ValueError("Expected `split_premixture` to be either " '"train", "val", or "test".')
        if snr_mixture is not None:
            if not (split_mixture in ("train", "val", "test")):
                raise ValueError("Expected `split_mixture` to be either " '"train", "val", or "test".')
        if split_reverb is not None:
            if not (split_reverb in ("train", "val", "test")):
                raise ValueError("Expected `split_reverb` to be either " '"train", "val", or "test".')
        ...
        # instantiate corpora
        self.corpus_s = df_librispeech.query(f"speaker_id in {self.speaker_ids}")
        if self.split_speech != "all":
            self.corpus_s = self.corpus_s.query(f'split == "{self.split_speech}"')

        if 0 < self.frac_speech < 1:
            self.corpus_s = self.corpus_s.sample(frac=frac_speech, random_state=0)
        ...
        self.corpus_m = None
        self.corpus_n = get_noise_corpus(corpus_mixture).query(f'split == "{self.split_mixture}"')

        # calculate maximum random offset for all utterances
        max_offset_func = lambda d: d.assign(max_offset=(sample_rate * d["duration"] - example_length)).astype(
            {"max_offset": int}
        )
        ...
        if self.dataset_duration:
            self.speech_data = wav_read_multiple(self.corpus_s.filepath, concatenate=True)
            self.speech_data = self.speech_data[: (self.dataset_duration * sample_rate)]
        ...
        self.add_noise = bool((snr_mixture is not None) and (self.len_n > 0))

        if not self.is_personalized and self.add_premixture_noise:
            raise ExperimentError("Non-personalized dataset contains " "premixture noise.")

        if self.dataset_duration and self.add_premixture_noise:
            raise ExperimentError("Fine-tuning dataset contains " "premixture noise.")
    ...
    def __call__(self, batch_size: int, seed: Optional[int] = None):
        ...
        indices = np.arange(batch_size * tmp_index, batch_size * (tmp_index + 1))
        s_filepaths = list(self.corpus_s.filepath.iloc[indices % self.len_s]) if self.len_s else []
        # m_filepaths = (list(self.corpus_m.filepath.iloc[indices % self.len_m])
        #               if self.len_m else [])
        n_filepaths = list(self.corpus_n.filepath.iloc[indices % self.len_n]) if self.len_n else []
        ...
        if self.add_premixture_noise:
            ...
            pre_snrs = tmp_rng.uniform(self.snr_premixture_min, self.snr_premixture_max, (batch_size, 1))
            x = p = mix_signals(s, m, pre_snrs)

        post_snrs = np.array([])
        if self.add_noise:
            n = wav_read_multiple(n_filepaths, seed=seed)
            post_snrs = tmp_rng.uniform(self.snr_mixture_min, self.snr_mixture_max, (batch_size, 1))
...
class ContrastiveMixtures(Mixtures):
    ...
    def __call__(self, batch_size: int, ratio_positive: float = 0.5, seed: Optional[int] = None):
        ...
        for i in range(0, batch_size, 2):
            ...
            if self.add_premixture_noise:
                if is_positive:
                    ...
                    pre_snr = [tmp_rng.uniform(self.snr_premixture_min, self.snr_premixture_max)] * 2
                else:
            ...
            if self.add_noise:
                if not is_positive:
                    ...
                    post_snr = [tmp_rng.uniform(self.snr_mixture_min, self.snr_mixture_max)] * 2
                else:
